...
export const location = {
  ...
  address: '',   // optional full postal address — shown only if filled in
  mapsUrl: '',   // paste a Google Maps share link here to make "Open Maps" use it directly
};
```

The embedded map on the site is a plain Google Maps **text search** for the
name (and address, if you add one) — no coordinates or map link are ever
invented. If you leave `mapsUrl` empty, the "Open Maps" button falls back to
the same text search opened in the Google Maps app/website. For a pin-perfect
result, add the full `address` and/or paste a real `mapsUrl` once you have one.
...
│   └── components/
│       ├── IntroGate.jsx        # "Tap to open" temple-door splash screen
│       ├── Hero.jsx
│       ├── Invitation.jsx
│       ├── Organizers.jsx        # Tap-through carousel with prev/next + dots
│       ├── Timeline.jsx           # Garland-string program, tap a day to expand
│       ├── Location.jsx            # Embedded map + "Open Maps" button
│       ├── Blessings.jsx            # Tap-to-shower-flowers interaction
│       ├── ArchFrame.jsx             # Ornate arch used around the Ganpati emblem
│       ├── GanpatiEmblem.jsx          # Original decorative Ganpati icon (SVG)
│       ├── Gallery.jsx
│       ├── Contact.jsx
│       ├── Footer.jsx
│       ├── MusicPlayer.jsx
...
## 13. The interactive touches

- **Intro gate** — visitors see a golden "tap to open" temple door first;
  tapping it reveals the invitation. This also matches how most phones
  require a real tap before they'll allow anything (including music) to play.
- **Organizer carousel** — swipe-free, just tap the `‹` `›` arrows or a dot
  to move between the three organizers.
- **Garland timeline** — tap any day's pill to expand its event details;
  day 1 is open by default so first-time visitors immediately see how it works.
- **Blessings / "पुष्पांजलि अर्पित करें"** — tapping the button showers
  animated flower petals over the Ganpati emblem, a small devotional moment
  before the gallery.
